...
def dncnn(input, is_training=True, output_channels=1):
    with tf.variable_scope('DnCNN', reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope('block19'):
...
class denoiser(object):
    def __init__(self, sess, sigma, cost_str, ckpt_dir, sample_dir, log_dir):
        ...
        #epsilon values
        self.eps_tf = 1.4*self.sigma_tf*0.0001
        ...
        self.Z = self.Y + tf.multiply(self.n, self.eps_tf)
        ...
        self.var = tf.square(self.sigma_tf/255.0, name = 'var_vector')
        self.var = self.var[:, tf.newaxis, tf.newaxis, tf.newaxis] #for vectorized calculation of the sure


        #######################################-COST FUNCTIONS-#######################################
        self.loss = (1.0 / batch) * tf.nn.l2_loss(self.Y_ - self.X) #MSE

        self.divergence = tf.multiply((1.0/self.eps_tf),tf.multiply(self.n, (self.Z_-self.Y_)))
        self.divergence_sum = tf.reduce_sum(tf.multiply(self.var, self.divergence))
        self.var_sum = self.dim1*self.dim2*tf.reduce_sum(self.var)/2.0

        self.sure = (1.0 / batch)*(tf.nn.l2_loss(self.Y - self.Y_) - self.var_sum + self.divergence_sum) #MC-SURE
    ...
    def train(self, data_path, sigma_path, data_noisy_path, eval_files, batch_size, epoch, lr):
        ...
        sigma_vector = np.load(sigma_path).astype('float32')
        data_noisy = np.load(data_noisy_path, mmap_mode='r')
        ...
        for epoch in range(start_epoch, epoch):
            ...
            for batch_id in range(0, numBatch):
                ...
                feed_dict_train = {self.X: batch_images, self.Y: batch_images_corrupt, self.sigma_tf: batch_sigma, self.lr: lr[epoch], self.is_training: True}
                ...
                # display the losses
                if (iter_num)%100==0:
                    feed_dict_test = {self.X: batch_images, self.Y: batch_images_corrupt, self.sigma_tf: batch_sigma, self.lr: lr[epoch], self.is_training: False}
